...
type DeployBookReq struct {
	...
	// BaseURI 用于 deployBook 第 4 个参数；空则用 Serial 或 env BASE_URI
...
func (h *FactoryHandler) DeployBook(w http.ResponseWriter, r *http.Request) {
	...
	backendAddr := gethcrypto.PubkeyToAddress(backendPriv.PublicKey)

	// -----------------------------
	// 链上 publisher 填谁（未传或未绑定则用 backend）
	// -----------------------------
	var publisherAddr common.Address
	if common.IsHexAddress(req.Publisher) {
		_, pubData, bindErr := h.getPublisherBind(ctx, req.CodeHash, req.Publisher)
		if bindErr == nil && strings.ToLower(strings.TrimSpace(pubData["role"])) == "publisher" {
			if addr := strings.ToLower(strings.TrimSpace(pubData["address"])); common.IsHexAddress(addr) && addr == strings.ToLower(req.Publisher) {
				publisherAddr = common.HexToAddress(req.Publisher)
			}
		}
	}
	if (publisherAddr == common.Address{}) {
		publisherAddr = backendAddr
		req.Publisher = strings.ToLower(backendAddr.Hex())
	}

	// -----------------------------
	// 1) 用 BACKEND_PRIVATE_KEY 代付部署子合约（调用工厂 deployBook，不扣 USDT）
	// -----------------------------
	txHash, err := h.deployByFactory(ctx, backendPriv, factoryAddr, req, publisherAddr)
	if err != nil {
		h.sendJSON(w, 500, map[string]interface{}{"ok": false, "error": "部署失败: " + err.Error()})
		return
	}

	// -----------------------------
	// 2) 等部署 receipt，解析新合约地址 bookAddr
	// -----------------------------
	receipt, err := h.waitReceipt(ctx, common.HexToHash(txHash), 60*time.Second)
	if err != nil {
		h.sendJSON(w, 500, map[string]interface{}{"ok": false, "error": "部署交易未确认: " + err.Error(), "txHash": txHash})
		return
	}
	if receipt.Status != 1 {
		revertReason := ""
		if receipt.BlockNumber != nil {
			revertReason, _ = h.getDeployBookRevertReason(ctx, factoryAddr, req, backendAddr, receipt.BlockNumber)
		}
		errMsg := fmt.Sprintf("部署交易失败 status=%d", receipt.Status)
		if revertReason != "" {
			errMsg = errMsg + ": " + revertReason
		}
		h.sendJSON(w, 500, map[string]interface{}{"ok": false, "error": errMsg, "txHash": txHash})
		return
	}

	bookAddr, err := h.parseBookAddrFromReceipt(ctx, factoryAddr, receipt)
	if err != nil || (bookAddr == common.Address{}) {
		h.sendJSON(w, 500, map[string]interface{}{"ok": false, "error": "无法从 receipt 解析新书合约地址: " + err.Error(), "txHash": txHash})
		return
	}
	...
	if err := h.saveBookMeta(ctx, bookAddr, req); err != nil {
		h.sendJSON(w, 500, map[string]interface{}{"ok": false, "error": "写入 Redis 失败: " + err.Error(), "txHash": txHash, "bookAddr": strings.ToLower(bookAddr.Hex())})
		return
	}
...
// deployBookRelayer 返回 deployBook 第 5 个参数（address）：RELAYER_ADDRESS 或 TREASURY_ADDRESS
func (h *FactoryHandler) deployBookRelayer() (common.Address, error) {
	for _, key := range []string{"RELAYER_ADDRESS", "TREASURY_ADDRESS"} {
		s := strings.TrimSpace(os.Getenv(key))
		s = strings.TrimPrefix(strings.ToLower(s), "0x")
		if s != "" && common.IsHexAddress("0x"+s) {
			return common.HexToAddress("0x" + s), nil
		}
	}
	return common.Address{}, errors.New("RELAYER_ADDRESS 或 TREASURY_ADDRESS 未设置")
}

// deployBookBaseURI 返回 deployBook 第 4 个参数（baseURI）：req.BaseURI 或 req.Serial 或 env BASE_URI
func (h *FactoryHandler) deployBookBaseURI(req DeployBookReq) string {
	if s := strings.TrimSpace(req.BaseURI); s != "" {
		return s
	}
	if s := strings.TrimSpace(req.Serial); s != "" {
		return s
	}
	return strings.TrimSpace(os.Getenv("BASE_URI"))
}

// deployFeeWei 解析 FACTORY_DEPLOY_FEE_CFX（支持小数如 0.01），单位 wei (1 ether = 1e18)
func deployFeeWei() *big.Int {
	feeStr := strings.TrimSpace(os.Getenv("FACTORY_DEPLOY_FEE_CFX"))
	if feeStr == "" {
		return big.NewInt(0)
	}
	...
	// 0.01 -> 0.01 * 1e18
	val := new(big.Float).SetFloat64(f)
	mult := new(big.Float).SetInt(new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil))
	val.Mul(val, mult)
	wei := new(big.Int)
	val.Int(wei)
	return wei
...
// -----------------------------
// Factory deployBook 部署（backend 代付签名）deployBook(string,string,string,string,address,uint256)
// -----------------------------
func (h *FactoryHandler) deployByFactory(ctx context.Context, backendPriv *ecdsa.PrivateKey, factory common.Address, req DeployBookReq, publisher common.Address) (string, error) {
	relayerAddr, err := h.deployBookRelayer()
	if err != nil {
		return "", err
	}
	baseURI := h.deployBookBaseURI(req)
	...
	factoryABI, err := abi.JSON(strings.NewReader(`[
		...
		],"name":"deployBook","outputs":[],"stateMutability":"payable","type":"function"}
	]`))
	if err != nil {
		return "", err
	}

	data, err := factoryABI.Pack("deployBook",
	...
	// 预检：eth_call 模拟执行，若会 revert 则直接返回原因，避免发交易
	msg := ethereum.CallMsg{From: fromAddr, To: &factory, Value: value, Data: data}
	if _, err := h.Client.CallContract(ctx, msg, nil); err != nil {
		if revertData, ok := ethclient.RevertErrorData(err); ok && len(revertData) > 0 {
			if reason, unpackErr := abi.UnpackRevert(revertData); unpackErr == nil {
				return "", fmt.Errorf("部署预检失败（合约将 revert）: %s", reason)
			}
		}
		return "", fmt.Errorf("部署预检失败: %w", err)
	}
...
// getDeployBookRevertReason 在指定区块用 eth_call 重放 deployBook，解析 revert 原因（用于 status=0 时给用户提示）
func (h *FactoryHandler) getDeployBookRevertReason(ctx context.Context, factory common.Address, req DeployBookReq, fromAddr common.Address, blockNumber *big.Int) (string, error) {
	relayerAddr, err := h.deployBookRelayer()
	if err != nil {
		return "", err
	}
	...
	value := deployFeeWei()
	msg := ethereum.CallMsg{From: fromAddr, To: &factory, Value: value, Data: data}
	_, err = h.Client.CallContract(ctx, msg, blockNumber)
	if err == nil {
		return "", nil
	}
...
// GetPublisherBalance 查询出版社余额：输入 codeHash，返回绑定地址原生代币余额；symbol 由 ChainID 决定（如 PAS/CFX/ETH）
func (h *FactoryHandler) GetPublisherBalance(w http.ResponseWriter, r *http.Request) {
	...
	h.sendJSON(w, 200, map[string]interface{}{
		"ok":      true,
		"address": address,
		"balance": balanceHuman.Text('f', 6),
		"symbol":  NativeTokenSymbol(h.ChainID),
	})
}
